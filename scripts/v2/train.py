...
from bdpan_sr.v2.optim import CosineAnnealingRestartLR
from bdpan_sr.v2.model import RRDBNet
from bdpan_sr.v2.dataset import SRDataset
from bdpan_sr.v2.loss import SRLoss
from bdpan_sr.v2.psnr_ssim import calculate_psnr, calculate_ssim


parser = argparse.ArgumentParser(description='sr v2 mod')
# data config
...
def train_step(trainer: Trainer, bat, bat_idx, global_step):
    ...
    loss_x2 = loss_func(pred_x2, bat_x2)
    loss_x4 = loss_func(pred_x4, bat_x4)
...
def val_step(trainer: Trainer, bat, bat_idx, global_step):
    ...
    psnr_ssim = (0.4 * psnr_x2 + 0.6 * psnr_x4) / 30 + (0.4 * ssim_x2 + 0.6 * ssim_x4)
    ...
    #     im_x = bat_x[i].numpy().transpose(1, 2, 0)
    #     im_x2 = bat_x2[i].numpy().transpose(1, 2, 0)
    #     im_x4 = bat_x4[i].numpy().transpose(1, 2, 0)
    #     im_pred_x2 = pred_x2[i].numpy().transpose(1, 2, 0)
    #     im_pred_x4 = pred_x4[i].numpy().transpose(1, 2, 0)
    #     psnr_x2_res.append(
    #         calculate_psnr(im_pred_x2, im_x2, crop_border=4, test_y_channel=True,)
    #     )
    #     ssim_x2_res.append(
    #         calculate_ssim(im_pred_x2, im_x2, crop_border=4, test_y_channel=True,)
    #     )
    #     psnr_x4_res.append(
    #         calculate_psnr(im_pred_x4, im_x4, crop_border=4, test_y_channel=True,)
    #     )
    #     ssim_x4_res.append(
    #         calculate_ssim(im_pred_x4, im_x4, crop_border=4, test_y_channel=True,)
    #     )
    # psnr_x2 = float(np.mean(psnr_x2_res))
    # ssim_x2 = float(np.mean(ssim_x2_res))
    # psnr_x4 = float(np.mean(psnr_x4_res))
    # ssim_x4 = float(np.mean(ssim_x4_res))
    # psnr_ssim = (psnr_x2 + psnr_x4) / 2 + (ssim_x2 + ssim_x4) / 2
    #
    # trainer.log({
    #     'val_loss': loss.item(),
    #     'val_loss_x2': loss_x2.item(),
    #     'val_loss_x4': loss_x4.item(),
    #     'val_psnr_x2': psnr_x2,
    #     'val_ssim_x2': ssim_x2,
    #     'val_psnr_x4': psnr_x4,
    #     'val_ssim_x4': ssim_x4,
...
def on_epoch_end(trainer: Trainer, ep):
    ...
    trainer.print(f'loss : {val_loss}, psnr_ssim : {psnr_ssim}, lr : {optimizer.get_lr()}')
