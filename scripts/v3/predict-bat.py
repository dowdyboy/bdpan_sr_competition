...
from bdpan_sr.v3.dataset import SRTestDatasetV2
...
def build_data():
    test_dataset = SRTestDatasetV2(src_image_dir, 6, use_normal=True, use_cache=True)
    return test_dataset
...
def test_step(model, bat, dataset):
    bat_im, bat_filepath, bat_hw = bat
    ...
    for i in range(len(bat_filepath)):
        cur_h, cur_w = bat_hw[i]
        cur_res_x2 = res_x2[i]
        cur_res_x4 = res_x4[i]
        cur_res_x2 = cur_res_x2[:, :cur_h*2, :cur_w*2]
        cur_res_x4 = cur_res_x4[:, :cur_h*4, :cur_w*4]
        cur_res_x2 = to_img_arr(cur_res_x2, dataset.un_normalize)
        cur_res_x4 = to_img_arr(cur_res_x4, dataset.un_normalize)
        Image.fromarray(cur_res_x2).save(
            os.path.join(save_x2_dir, os.path.basename(bat_filepath[i]))
        )
        Image.fromarray(cur_res_x4).save(
            os.path.join(save_x4_dir, os.path.basename(bat_filepath[i]))
        )


def main():
    test_dataset = build_data()
    model_chk_path = 'checkpoint/v3/best_epoch_341/model_0.pd'
    model = build_model()
    ...
    for i in range(len(test_dataset)):
        with paddle.no_grad():
            test_step(model, test_dataset[i], test_dataset)
