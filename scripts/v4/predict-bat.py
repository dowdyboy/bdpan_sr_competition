...
from bdpan_sr.v4.dataset import SRTestDatasetV2
...
def test_step(model, bat, dataset):
    ...
    for i in range(len(bat_filepath)):
        ...
        save_img_arr(cur_res_x2, os.path.join(save_x2_dir, os.path.basename(bat_filepath[i])))
        save_img_arr(cur_res_x4, os.path.join(save_x4_dir, os.path.basename(bat_filepath[i])))
