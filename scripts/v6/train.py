...
from bdpan_sr.v6.optim import CosineAnnealingRestartLR
from bdpan_sr.v6.model import SwinIRPlus
from bdpan_sr.v6.dataset import SRDataset
from bdpan_sr.v6.loss import SR2StageLoss
from bdpan_sr.v6.psnr_ssim import calculate_psnr, calculate_ssim


parser = argparse.ArgumentParser(description='sr v6')
# data config
...
parser.add_argument('--img-size', type=int, default=128, help='input img size')  # 424
# optimizer config
...
def build_model():
    # model = SwinIRPlus(
    #     img_size=args.img_size, mlp_ratio=3, embed_dim=126, up_dim=64, patch_size=2,
    #     depths=[6, 8, 16, 6], num_heads=[3, 3, 6, 3],
    # )
    model = SwinIRPlus(img_size=args.img_size, mlp_ratio=4, embed_dim=96, up_dim=48,
...
def main():
    ...
    model = build_model()
    loss_func = SR2StageLoss(step_per_epoch=3000 // args.batch_size, epoch=20)
